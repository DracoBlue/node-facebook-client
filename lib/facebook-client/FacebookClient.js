...
function doRequest(options) {
    return function(cb) {
        options = options || {};
        if (!(options.host || options.port || options.path || options.parser)) {
            throw new Error('Cannot call doRequest without host, port, parser or path!');
        }
...
var FacebookClient = function(api_key, api_secret, options) {
    ...
    var doRestCall = function(method, params, access_token) {
        ...
        return doRawJsonRequest({
            'host': self.options.facebook_server_host,
            'port': self.options.facebook_server_port,
            'path': request_path_array.join(''),
            'secure': true
        });
    };
